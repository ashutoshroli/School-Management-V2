...
/**
 * Whether Google OAuth is actually configured for this deployment.
 * `passport-google-oauth20`'s Strategy constructor throws synchronously
 * if clientID/clientSecret are empty strings - without this guard, the
 * entire server would fail to start (not just the OAuth feature) on
 * any deployment/trial environment that hasn't set up Google OAuth yet.
 * Email/password login (the primary login method for staff) works
...
if (isGoogleOAuthConfigured()) {
  ...
  console.warn(
    "[auth] Google OAuth is not configured (missing GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRET) - " +
      "the /api/auth/google routes will return an error, but email/password login is unaffected."
  );
}
